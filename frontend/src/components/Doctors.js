...
export default function DoctorDashboard() {
  ...
  const fetchExistingPrescriptions = async (patientId) => {
    ...
    try {
      ...
      const response = await fetch(BASE_API+ `/api/doctor/prescriptions/${patientId}`, {
        headers: {
  ...
  const fetchDoctorProfile = async () => {
    try {
      ...
      const response = await fetch(BASE_API+ '/api/doctor/profile', {
        headers: {
          'Authorization': `Bearer ${token}`
        }
      });
  ...
  const fetchPatientsWithAppointments = async () => {
    try {
      ...
      const response = await fetch(BASE_API+ '/api/doctor/patients-with-appointments', {
        headers: {
          'Authorization': `Bearer ${token}`
        }
      });
  ...
  const fetchAppointments = async () => {
    try {
      ...
      const response = await fetch(BASE_API+ '/api/doctor/appointments', {
        headers: {
          'Authorization': `Bearer ${token}`
        }
      });
  ...
  const renderPatientManagement = () => {
    ...
    const fetchAvailableSlots = async (patientId, date) => {
      ...
      try {
        const token = localStorage.getItem('token');
        const response = await fetch(`${BASE_API}/api/doctor/available-slots?patientId=${patientId}&date=${date}`, {
          headers: {
            'Authorization': `Bearer ${token}`
          }
        });
    ...
    const handleDeletePrescription = async (prescriptionId) => {
      if (window.confirm('Are you sure you want to delete this prescription?')) {
        try {
          const token = localStorage.getItem('token');
          const response = await fetch(`${BASE_API}/api/doctor/prescriptions/${prescriptionId}`, {
            method: 'DELETE',
            headers: {
              'Authorization': `Bearer ${token}`
            }
      ...
      if (selectedAction === 'prescribe-medication') {
        try {
          ...
          const url = appointmentData.prescriptionId
            ? `${BASE_API}/api/doctor/prescriptions/${appointmentData.prescriptionId}`
            : BASE_API+ '/api/doctor/prescribe-medication';
        ...
        try {
          const token = localStorage.getItem('token');
          const response = await fetch(BASE_API+ '/api/doctor/schedule-appointment', {
            method: 'POST',
            headers: {
              'Content-Type': 'application/json',
              'Authorization': `Bearer ${token}`
        ...
        <CardContent>
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="space-y-2">
              ...
              <Select id="patient" name="patientId" value={appointmentData.patientId} onChange={handleInputChange} required >
            ...
            {selectedAction === 'schedule-appointment' && (
              <>
                ...
                <div className="space-y-2">
                  ...
                  <Select id="time" name="time" value={appointmentData.time} onChange={handleInputChange} disabled={availableSlots.length === 0} required>
                ...
                <div className="space-y-2">
                  ...
                  <Input id="reason" name="reason" value={appointmentData.reason} onChange={handleInputChange} placeholder="Brief description of your concern" required />
                </div>
              </>
            )}
            {selectedAction === 'prescribe-medication' && (
              <>
                ...
                <div className="space-y-2">
                  ...
                  <Input id="medication" name="medication" value={appointmentData.medication || ''} onChange={handleInputChange} placeholder="Medication name" required />
                </div>
                <div className="space-y-2">
                  <Label htmlFor="dosage">Dosage</Label>
                  <Input id="dosage" name="dosage" value={appointmentData.dosage || ''} onChange={handleInputChange} placeholder="Dosage" required />
                </div>
                <div className="space-y-2">
                  <Label htmlFor="frequency">Frequency</Label>
                  <Input id="frequency" name="frequency" value={appointmentData.frequency || ''} onChange={handleInputChange} placeholder="Frequency" required />
                </div>
              </>
            )}
